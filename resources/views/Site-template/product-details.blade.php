...
<html lang="en">
    <head>
        ...
        <!-- Favicon-->
        <link rel="icon" type="image/x-icon" href="{{ asset('/') }}product-detail/assets/favicon.ico" />
        <!-- Bootstrap icons-->
     
        <!-- Core theme CSS (includes Bootstrap)-->
        <link href="{{ asset('/') }}product-detail/css/styles.css" rel="stylesheet" />
    </head>
    <style>
    ...
    <div class="row no-gutters">
        <div class="col-md-6 pr-2">
            <div class="card">
                <div class="demo">
                    <ul id="lightSlider">
                    <li data-thumb="{{asset($product->image)}}"> <img src="{{asset($product->image)}}" /> </li>
                        @foreach($subimages as $key => $subimage)
                        <li data-thumb="{{asset($subimage->subimage)}}"> <img src="{{asset($subimage->subimage)}}" /> </li>
                        @endforeach
                    </ul>
                </div>
            </div>
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="about"> <span class="font-weight-bold">{{$product->Product_name}} </span>
                    <h4 class="font-weight-bold">${{$product->Product_Price}} </h4>
                </div>
                <form action="{{ route ('cart')}}" method="POST">
                    @csrf
                <label  class="form-label h5 mr-3" for="">Quantity: </label>
                <input name="qnty" class="form-control mb-2 d-inline " style="width: 150px;" type="number"value="1" min="1">
            ...
            <div class="card mt-2"> <span>Similar items:</span>
                ...
                <a style="text-decoration: none;" href="{{route('product-details',['id'=> $related_product->id])}}" >
                <div class="card border p-1" style="width: 9rem;"> <img src="{{asset($related_product->image)}}" hight="10px" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h6 class="card-title">${{$related_product->Product_Price}}</h6>
                        </div>
                    </div>
                    </a>
                    @endif
